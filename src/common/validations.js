...
export default {
  ...
  numerosMayorACero: v => {
    const re = /^[1-9]\d*$/
    return re.test(String(v)) || !v || 'Solo se permiten números enteros mayores a cero.'
  },
  decimales: v => {
  ...
  arrayRequerido: v => {
    return (Array.isArray(v) && v.length > 0) || 'Debe seleccionar al menos una entidad.'
  },
  requeridoBlanco: v => {
    return (!!v && v.trim().length > 0) || 'El campo es requerido.'
  }
}
